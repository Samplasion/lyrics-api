...
const searchHeaders = {
    ...
    Host: 'genius.com',
    Accept:
        'application/x-javascript, text/javascript, text/html, application/xml, text/xml, */*',
};
    ...
    async lyrics(song: PartialSong, firstTime = true): Promise<Song> {
        ...
        const artist = $('h2')
            ...
            lyrics = $('.lyrics')
                ...
                .text()
                .trim(),
            thumbnail = $('.cover_art-image')
                .first()
                .attr('src');

        if (!(artist && lyrics && thumbnail)) {
            return firstTime
                ? this.lyrics(song, false)
                : {
