...
export default class AZLyricsProvider extends Provider {
    public async search(query: string) {
        const body = await phin({
            url: `https://search.azlyrics.com/search.php?q=${encodeURIComponent(
                query
            )}`,
        ...
        const length = parseInt(
            $('.panel-heading')
                .first()
                .text()
                .match(/\[1-(\d+) of \d+ found\]/)?.[1] ?? '0'
        );
        return Array.from({ length }).map((_never, i) => {
            const row = $(
                'body > div.container.main-page > div > div > div:nth-child(1) > table > tbody > tr'
            ).eq(i);
            return {
                title:
                    row
